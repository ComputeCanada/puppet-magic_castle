...
GPU_MEM_SIZE=$(/usr/sbin/lspci -d 10de: | grep -m1 -oP '\[*\K[0-9]*[T|G](?=B)' | numfmt --from=iec)
# Grep the actual GPU memory size from lspci verbose and convert to bytes
AVAIL_MEM_SIZE=$(/usr/sbin/lspci -d 10de: -v | grep -m1 -oP 'Memory.*\(64-bit, prefetchable\).*\[size=\K([0-9]*[T|G])(?=])' | numfmt --from=iec)
